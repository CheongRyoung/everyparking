...
var Grid = function(tblEl, options, searchApiUrl, searchFunc){
    ...
    this.drawPaging = function(){
        ...
        if($this.$pagingEl) {
            ...
            html += `<ul>`;
            html += `    <li>`;
            html += `        <button type="button" onclick="girdPageMove(${$this.pagingOpt.page-1})" ${$this.pagingOpt.page == 1 ? 'disabled="disabled"' : ''}"><span class="icon arrow prev"></span></button>`;
            html += `    </li>`;
            html += `    <li>`;
            html += `        <span>${this.pagingOpt.page}</span> / <span>${this.pagingOpt.max}</span>`;
            html += `    </li>`;
            html += `    <li>`;
            html += `        <button type="button"  onclick="girdPageMove(${$this.pagingOpt.page+1})" ${$this.pagingOpt.page == $this.pagingOpt.max ? 'disabled="disabled"' : ''}"><span class="icon arrow next"></span></button>`;
            html += `    </li>`;
            html += `</ul>`;
