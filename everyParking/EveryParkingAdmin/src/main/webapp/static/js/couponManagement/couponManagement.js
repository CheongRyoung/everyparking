let couponManagement = {
	    ...
	    initPage: function(){
	        ...
	        this.gridOption = {
	            cols : [
	                {title : "이미지", name: "FILE_SEQ", filter:function(data, rowData, ridx, cidx, $this) {
                	 return `<img src="/uploadImage/${rowData.FILE_URL}/${rowData.FILE_CONV_NAME}" style="height:30px; width: auto;"/>`}},
	                {title : "쿠폰명", name: "COU_NAME"},
	                {title : "할인가격", name: "COU_PRICE", type:"number", colWidth:"20"},
	                ...
	                {title : "삭제", name: "COU_SEQ", filter:function(data, rowData, ridx, cidx, $this) {
	                    return `<button class="btn btn-outline-danger btn-sm" onclick="couponManagement.deleteCoupon(${ridx})">삭제</button>`
	                }, colWidth:"10"}
	            ],
	        ...
	        this.grid = new Grid("#couponManageTable", $this.gridOption,"/couponManagement/selectListCoupon");
	    },
